...
**Детерминизм без моков.** В чёрном ящике подменить зависимости нельзя — детерминизм даёт конфигурация стенда, зафиксированная в справочном файле `stand/.env.stand` (ничего не запускает; правится только отдельным MR). Ключевые следствия: register сразу возвращает JWT (`EMAIL_VERIFICATION_ENABLED=false`), LLM-агенты работают эвристикой (пустые API-ключи, тесты ассертят `model_used == "heuristic"`), лимиты и TTL берутся из этого файла, а не из дефолтов бэкенда.

**Изоляция уникальностью, а не очисткой.** База между тестами не чистится; набор параллелизуем (`pytest -n`). Каждый тест получает своего пользователя `t-{uuid}@example.com` и свой IP в `X-Forwarded-For` (персональные бакеты rate-limit без флаша Redis и без sleep).
...
Хуки (`.claude/settings.json`): пока существует маркер `.claude/tmp/api-tests-active`, запись разрешена только под `tests/`, `plans/`, `docs/`, `.claude/tmp/` (защита `stand/.env.stand` от «подкрутить стенд, чтобы позеленело» — FORBID-010/011); каждый записанный `.py` автоматически форматируется `ruff format`.
