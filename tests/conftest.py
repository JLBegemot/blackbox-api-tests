...
def client_ip() -> str:
    ...
    ``rate_limit_dep`` бэкенда ключует IP-бакеты по первому хопу
    ``X-Forwarded-For`` — уникальный IP даёт тесту персональные счётчики
    лимитов без флаша Redis и без ``sleep``.
    """
