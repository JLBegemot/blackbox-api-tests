"""Регистрация при выключенной верификации e-mail.

На стенде ``EMAIL_VERIFICATION_ENABLED=false`` — это конфигурация dev-контура:
register активирует аккаунт мгновенно и сразу возвращает JWT (кейс заменяет
белоящичный «register не возвращает токен», который был поведением флага
``true`` — см. план 08 §2.5).
"""
